...
var InlinePopupForm = {
	open : function(url, container) {
		...
		if(url) {
			...
			Sarissa.updateContentFromURI(HTTP.setURLParams({'rxhtml':'true'}, url), $(container), null, function(oNode, oTargetElement) {
	...
	submit : function(node, skipReload, successCallback) {
		...
		Sarissa.formPostRequest(HTTP.setURLParams({'rxhtml':'true'}, form.action), container, form, callback);
	},
		...
		function sarissa_dhtml_loadHandler() {
			if(Sarissa.validateXHRResponse(xmlhttp)) {
				...
				if(!success && oTargetElement) {
					...
					ampFix(sFromUrl, oTargetElement);
				}
...
function showCountryAddressForm(country, countryLists, initialCountry) {
	...
	// hack to force last name fields to be disabled when editing main contact
	if(initialCountry) {
		...
		for(field in lastNameFields)
			if(lastNameFields[field])
				lastNameFields[field].disabled = true;
	}
}

function showCardTip(type) {
	if(type != null) {
		Tooltip.showTip($('cvvCodeHelp' + type).innerHTML);
	} else
		showCardTip({
			'AX':'Amex',
			'DI':'Disc'
